#!/usr/bin/env python3
"""Regulator ranking audit — validates and hardens the regulator ranking.

No downloads, no PPLs, no MCMC, no new dependencies. It just makes the ranking
more defensible with: (1) baseline comparison, (2) bootstrap stability,
(3) global vs. condition-specific regulators.
...
Outputs:
    ...
    (+ adds stability_frequency and regulator_class to top_regulators_for_review.csv)
"""
...
from model_hubs import fit_fixed_effects   # reuses the fixed-effects GLM
...
SIGMA2E = None   # global noise variance (set from all rows)


def eb_scores(sub):
    """EB posterior mean of the per-gene effect (u_g) for a set of rows."""
    g = sub.groupby("gene")
    d = g["work"].mean()
    n = g.size()
    s2 = SIGMA2E / n
...
def main():
    ...
    sig = de[de["ontarget_significant"]].copy()          # eligible rows (KD gate)
    print(f"  total rows={len(de):,} · eligible (KD sig)={len(sig):,} · sigma2_e={SIGMA2E:.3f}")

    # ---------- 1. BASELINE COMPARISON ----------
    raw = de.groupby("gene")["n_downstream"].max()                     # ignores KD
    kdg = sig.groupby("gene")["n_downstream"].max()                    # peak among sig
    eb = eb_scores(sig)                                                # EB with gate
    ...
    comp["dropped_by_kd_gate"] = comp["kd_gated_score"].isna()          # raw hub without valid KD
    ...
    print(f"  [1] baseline comparison → ranking_baseline_comparison.csv "
          f"({n_drop}/{TOP_N} raw hubs drop at the KD gate)")

    # figure 08 — 3-column slopegraph: raw -> KD-gated -> EB
    ...
    for gene, r in top_raw.iterrows():
        ...
        if r["dropped_by_kd_gate"]:
            col = UP                                    # drops at the KD gate
        elif int(r["eb_rank"]) <= TOP_N:
            col = ACCENT                                # survives, robust
        else:
            col = AMBER                                 # demoted by shrinkage (condition-specific)
        ...
        if col is ACCENT:
            ...
            ax.text(2.06, y2, "no valid KD", ha="left", va="center", fontsize=7.5, color=col)
    ax.axhspan(SINK - 1.3, SINK + 1.3, color=UP, alpha=.06)
    ax.text(2.06, SINK, "" , fontsize=7)
    ax.text(1.0, SINK + 2.6, "yellow = condition-specific hub demoted by EB shrinkage",
    ...
    ax.set_yticks([1, 10, 20, 30, 40]); ax.set_ylabel("rank position (top-40; ▼ = out)")
    ax.set_title("Why quality gates matter", fontweight="bold", fontsize=12)
    ax.grid(False)
    fig.tight_layout(); fig.savefig(FIG / "08_kd_gate_changes_ranking.png", dpi=135, bbox_inches="tight")
    plt.close(fig)
    print("      figure → docs/figures/08_kd_gate_changes_ranking.png")
    ...
    # figure 11 — stability of the top-30 by point EB
    ...
    ax.set_xlabel("selection frequency in the top-30  (bootstrap B=200)")
    ax.set_title("Ranking stability — does the gene stay in the top?", fontweight="bold", fontsize=11)
    ax.grid(axis="y")
    fig.tight_layout(); fig.savefig(FIG / "11_ranking_stability.png", dpi=135, bbox_inches="tight")
    plt.close(fig)
    print("      figure → docs/figures/11_ranking_stability.png")
    ...
    # full class table (all genes) — for the API filter
    ...
    # figure 10 — scatter specificity vs regulatory power
    ...
    ax.set_xlabel("condition_specificity = max / sum of n_downstream (KD sig)")
    ax.set_ylabel("EB regulatory power (u_g)")
    ax.set_title("Global vs. context-specific regulators", fontweight="bold", fontsize=11)
    ax.legend(frameon=False, loc="lower right")
    fig.tight_layout(); fig.savefig(FIG / "10_global_vs_context_specific.png", dpi=135, bbox_inches="tight")
    plt.close(fig)
    print("      figure → docs/figures/10_global_vs_context_specific.png")

    # ---------- 4. enrich top_regulators_for_review.csv ----------
